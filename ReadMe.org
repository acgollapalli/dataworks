...
Try and get that same enpoint after having made the changes, it should return the new value!
...
Now let's say you want to add another node. All you have to do is point it at a running kafka broker in your config.edn and your database, stream-processors, api endpoints will automagically stay in sync.
